...
class Line(object):
    ...
    def length(self):
        ...
        Type hints:
          :rtype: float
        """
        length = math.sqrt((self.end.x - self.start.x)**2 + (self.end.y -
                                                self.start.y)**2)
        return length
        # --------------------------------------------------------------
        # DONE: 7.
    ...
    def is_parallel(self, line2):
        ...
        if (line2.end.x - line2.start.x) == 0:
            return False
        elif (self.end.x - self.start.x) and (line2.end.x - line2.start.x) \
                == 0:
            return True
        slope_self = (self.end.y - self.start.y) / (self.end.x - self.start.x)
        slope_line2 = (line2.end.y - line2.start.y) / (line2.end.x - line2.start.x)
        if slope_self == slope_line2:
            return True
        else:
            return False
